...
private:
    ...
    std::shared_ptr<ComponentArray<T>> getComponentArray() {
        ...
        assert(m_componentArrays.count(typeName) && "Component type not registered before use.");
...
public:
    ...
    bool hasComponent(EntityID entity) {
        // 직접적인 확인 방법이 없으므로, getComponentArray를 호출하고
        // 그 안의 맵에 엔티티가 있는지 확인해야 합니다. (최적화 가능)
        // 여기서는 간단하게 구현합니다.
        const char* typeName = getComponentTypeName<T>();
        if (m_componentArrays.find(typeName) == m_componentArrays.end()) {
            return false;
        }
        return std::static_pointer_cast<ComponentArray<T>>(m_componentArrays[typeName])->hasEntity(entity);
    }

    // 시스템을 위한 뷰(View) 제공 - 간단한 버전
    // 특정 컴포넌트를 가진 모든 엔티티를 순회하고 싶을 때 사용
    template<typename T>
    std::vector<EntityID> view() {
        return getComponentArray<T>()->getAllEntities(); // ComponentArray에 getAllEntities() 함수 추가 필요
    }
};