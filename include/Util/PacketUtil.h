...
inline void WriteHeader(uint8_t*& beginPtr, enum PACKET packetID,
                        std::size_t size) {
  Write16BigEnd(beginPtr, static_cast<uint16_t>(packetID));
  Write16BigEnd(beginPtr, static_cast<uint16_t>(size));
}

inline void GetHeader(const uint8_t*& beginPtr, enum PACKET& packetID,
                      std::size_t& size) {
