...
이 프로젝트는 C++ 언어를 깊이 파고들며 가장 효율적인 게임 시스템 설계를 고민하고 실제로 구현하며 그 원리를 체득하기 위해 시작하였습니다. 특히 서버 프로그래머라는 목표를 세운 후, 2주 남짓한 짧은 기간 안에 기존의 싱글플레이 ECS 엔진 위에 IOCP 기반의 TCP 비동기 네트워크 시스템을 통합하는 도전을 진행했습니다. 이 과정을 통해 복잡한 시스템을 확장하는 능력과 대규모 온라인 게임에 필요한 아키텍처에 대한 깊은 이해를 얻을 수 있었습니다.
...
- **네트워크 효율성 증대**
  ...
  - **Nagle 알고리즘 비활성화**: Nagle 알고리즘은 작은 패킷들을 모아서 한 번에 전송함으로써 네트워크 오버헤드를 줄이는 기법입니다. 그러나 실시간 게임 환경에서는 이 알고리즘으로 인한 지연(latency)이 게임 응답성을 저해합니다. Nagle 알고리즘을 비활성화함으로써 낮은 지연시간, 실시간 상호작용 개선, 패킷 전소 시간이 일관되게 유지되어 예측 가능한 네트워크 동작을 얻을 수 있었습니다.
...
[![server-client-preview](./assets/docs/server-client-preview.gif)](https://www.youtube.com/watch?v=DAUPmR57i-o)
...
현재 구현된 IOCP 기반 TCP 서버의 안정성과 확장성 개선:
