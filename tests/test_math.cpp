...
bool test_map_range() {
    ...
    if (result != 50) {
        std::cerr << "map_range test failed: expected 50, got " << result << std::endl;
        return false;
    }
    
    auto result2 = util::map_range(0.5f, 0.0f, 1.0f, 100.0f, 200.0f);
    if (std::abs(result2 - 150.0f) > 0.001f) {
        std::cerr << "map_range test 2 failed: expected 150, got " << result2 << std::endl;
        return false;
    }
...
bool test_distance() {
    ...
    if (std::abs(dist - 5.0) > 0.001) {
        std::cerr << "distance test failed: expected 5.0, got " << dist << std::endl;
        return false;
    }
