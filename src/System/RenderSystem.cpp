...
void RenderSystem::RenderChunks() {
  ...
  for (EntityID entity : chunkView) {
    ...
    Vec2f screenPos = WorldToScreen(transform.position, cameraPos, screenWidth, screenHeight);
    ...
    if (chunk.chunkTexture) {
      SDL_Rect destRect = {
          ...
          CHUNK_WIDTH * TILE_PIXEL_WIDTH,
          CHUNK_HEIGHT * TILE_PIXEL_HEIGHT
      };
...
Vec2f RenderSystem::WorldToScreen(Vec2f worldPos, Vec2f cameraPos, int screenWidth, int screenHeight) const {
  return {
